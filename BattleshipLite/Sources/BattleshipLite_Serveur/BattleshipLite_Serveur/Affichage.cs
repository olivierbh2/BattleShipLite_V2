...
namespace BattleshipLite_Serveur
{
    public class Affichage
    {
        ...
        public static void MessageVictoire(Partie partie)
        {
            Joueur joueur1 = partie.Joueurs[0];
            Joueur joueur2 = partie.Joueurs[1];

            
            if (joueur2.Plateau.Bateaux.All(bateau => bateau.Positions.All(c => c.EstTouche)))
            {
                Console.WriteLine($"{joueur1.Nom} a gagné ! Tous les bateaux de {joueur2.Nom} sont coulés.");
            }
            else if (joueur1.Plateau.Bateaux.All(bateau => bateau.Positions.All(c => c.EstTouche)))
            {
                Console.WriteLine($"{joueur2.Nom} a gagné ! Tous les bateaux de {joueur1.Nom} sont coulés.");
            }
        }
